...
(function () {
  ...
  // Polyfill cho ctx.roundRect nếu trình duyệt cũ chưa hỗ trợ
  ...
  function handleCanvasTouches(e) {
    ...
    if (leftPressed) btnLeft.classList.add("active");
    else btnLeft.classList.remove("active");
    if (rightPressed) btnRight.classList.add("active");
    else btnRight.classList.remove("active");
  }

  canvas.addEventListener("touchstart", handleCanvasTouches, { passive: true });
  canvas.addEventListener("touchmove", handleCanvasTouches, { passive: true });
  canvas.addEventListener("touchend", handleCanvasTouches, { passive: true });
  canvas.addEventListener("touchcancel", handleCanvasTouches, { passive: true });
  ...
  class BubbleParticle {
    ...
    update(deltaSec) {
      const dt = (deltaSec !== undefined ? deltaSec : 0.016) * 60;
      this.wobble += 0.1 * dt;
      this.x += (this.vx + Math.sin(this.wobble) * 0.5) * dt;
      this.y += this.vy * dt;
  ...
  function renderSkinGrid() {
    ...
    SKINS.forEach((skin) => {
      ...
      slot.innerHTML = `
                    ...
                    <span class="slot-name ${rarityClass}">${skin.name.replace(/\r?\n/g, "<br>")}</span>
  ...
  function showSingleResult(skin) {
    gachaResultArea.innerHTML = `
                <div class="gacha-single-result ${rarityClass(skin)}">
                    ...
                    <span class="result-name">${skin.name.replace(/\r?\n/g, " ")}</span>
  ...
  function doPull(count) {
    ...
    isPulling = true;
    btnPull1.disabled = true;
    btnPull10.disabled = true;
